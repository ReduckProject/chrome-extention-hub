...
const specs = [
  ...
  ['status', 'Fast cached status for one or all ChatGPT tabs, including profile-wide accessPause on a website rate limit. Refresh reads existing page state with a 2 second per-tab bound; never reloads conversations or starts image loading.', schema({ tabKey, refresh: { type: 'boolean', default: false } }), true],
