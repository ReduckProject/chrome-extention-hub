...
const attachments = { type: 'array', maxItems: 10, description: 'Images or files from local paths OR inline bytes; no temporary file needed for inline data. At most 10 files and 20 MiB total decoded bytes. Reuse identical content on requestId retries. Upload runs return immediately; poll runId. Requires adapter 45/content 6.',
  items: { oneOf: [
...
const specs = [
  ...
  ['access', 'Wait for automatic access recovery without ending the task. A rate-limit pause lasts five minutes, then the service probes current pages and acknowledges only the old rate-limit dialog. If recovery fails, wait another five minutes. Use action:wait with timeoutMs:25000 repeatedly until accessPause is null, then CONTINUE the original step and all remaining prompts with original IDs. A timeout does not end the task. action:resume requests the same check when due; it cannot skip backoff. Recovery does not blindly replay prompts or prove website recovery.', schema({ profileId: string('Exact profileId, required with multiple connections.'), action: { type: 'string', enum: ['status', 'pause', 'resume', 'wait'], default: 'status' }, timeoutMs: { type: 'integer', minimum: 0, maximum: 25000, default: 25000, description: 'Bounded local wait; repeat while accessPause exists, keeping the task alive.' } })],
  ...
  ['send', 'Submit text, images or files to an explicitly identified idle ChatGPT tab. Provide prompt and/or attachments. The page must confirm uploads before sending; upload errors preserve the draft. Requires a stable requestId; retry with the SAME ID and identical files after timeout. Returns promptly with a persisted run ID. Completion means the web response ended, including text-only or refusal responses; kind describes intent, not the completion condition.', schema({ tabKey, prompt: string('Exact prompt text; may be omitted when attachments are provided.'), attachments, requestId: string('Unique logical submission ID, 4–150 characters; reuse only for the identical request.'), kind: { type: 'string', enum: ['image', 'text'], default: 'text' }, expectedModel: string('Optional exact current model picker label; aborts before sending if it changed.') }, ['tabKey', 'requestId'])],
...
function compact(method, result) {
  if (['tabs', 'status'].includes(method) && result.tabs) result.tabs = result.tabs.map(tab => ({
    ...
    surface: tab.surface, composerReady: tab.composerReady, draftLength: tab.draftLength, attachmentCount: tab.attachmentCount || 0, observationError: tab.observationError, bootstrapError: tab.bootstrapError,
  }));
