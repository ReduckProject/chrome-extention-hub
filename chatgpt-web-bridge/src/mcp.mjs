...
const specs = [
  ...
  ['status', 'Fast cached status including profile-wide accessPause. Refresh reads DOM without reloading. diagnostics:true with one exact tabKey reads existing browser resource timing and recent bridge operations, never sends website requests. Buffered timings may be incomplete and cannot alone attribute a request to the bridge.', schema({ tabKey, refresh: { type: 'boolean', default: false }, diagnostics: { type: 'boolean', default: false, description: 'Inspect existing request timing metadata for one exact tabKey; no website requests, URLs exclude query values.' } }), true],
  ...
  ['wait', 'Wait locally at most 25 seconds for a state revision. Terminal runs, awaiting_user and accessPause return immediately; do not loop wait during a rate-limit backoff. completed means the response ended regardless of images. A timeout is not a generation failure.', schema({ runId, afterRevision: { type: 'integer', minimum: 0 }, timeoutMs: { type: 'integer', minimum: 0, maximum: 25000, default: 20000 } }, ['runId']), true],
];
