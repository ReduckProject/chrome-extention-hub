...
export const toolDefinitions = specs.map(([name, description, inputSchema, readOnly]) => ({
  name: `chatgpt_${name}`, description: description +
    (name === 'tabs' ? ' Creation requires a task lease, count:1, known inventory and fewer than four current tabs; the task then reuses that tab.' :
      ['new_chat', 'models', 'select_model', 'send', 'download', 'recover_images', 'stop'].includes(name) ? ' Requires your active task leaseId. PROFILE_COOLDOWN includes retryAfterMs and means no page command was sent.' :
...
const server = new Server({ name: 'chatgpt-web-bridge', version: '0.2.0' }, { capabilities: { tools: {} } });
