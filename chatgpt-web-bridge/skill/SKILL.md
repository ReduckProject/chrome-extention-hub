...
服务端 0.2.0 起，同一 Chrome profile 的整个任务由 MCP 统一串行调度。按 [任务占用与队列协议](references/task-leases.md) 执行，不能只靠 tab 的 idle 状态认领页面。
...
- 同一 profile 的并行需求由服务端队列串行执行，如实说明实际执行方式；不通过多个客户端或多个 tab 绕过占用。每个任务单独保存提示词、模型、taskId、leaseId、tabKey、runId、聊天链接及 openedByThisTask，结束后按来源清理并释放占用。
